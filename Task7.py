...
res = []
with open('text_77.json', 'a', encoding='utf-8') as write_f:
    with open('text_7.txt', encoding='utf=8') as f_obj:
        p = {}
        for line in f_obj:
            p[line.split(" ")[0]] = int(line.split(' ')[2]) - int(line.split(' ')[3])
        a_p = sum([int(i) for i in p.values() if int(i) > 0]) / len([int(i) for i in p.values() if int(i) > 0])
        res.append(p)
        res.append(round(a_p))
